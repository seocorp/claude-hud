...
type CachedScopedEntry = {
  label: string;
  percent: number | null;
  resets_at: string | null;
};

type ScopedUsageCache = {
  checked_at: number;
  scoped: CachedScopedEntry[];
};
...
function readAccessToken(configDir: string): string | null {
  try {
    ...
    const token = (JSON.parse(raw) as { claudeAiOauth?: { accessToken?: unknown } })
...
function parseScopedLimits(body: unknown): CachedScopedEntry[] {
  const limits = (body as { limits?: unknown })?.limits;
  if (!Array.isArray(limits)) {
    return [];
  }

  const scoped: CachedScopedEntry[] = [];
...
function reviveScoped(entries: CachedScopedEntry[]): ModelScopedUsage[] {
...
function readCache(cachePath: string): ScopedUsageCache | null {
  try {
    const parsed = JSON.parse(fs.readFileSync(cachePath, 'utf8')) as ScopedUsageCache;
    if (typeof parsed?.checked_at !== 'number' || !Array.isArray(parsed.scoped)) {
      return null;
    }
    return {
      checked_at: parsed.checked_at,
      scoped: parsed.scoped.filter(
        (entry): entry is CachedScopedEntry =>
...
function writeCache(cachePath: string, cache: ScopedUsageCache): void {
...
export async function getModelScopedUsage(
  overrides: Partial<ScopedUsageDeps> = {},
): Promise<ModelScopedUsage[] | null> {
  const deps: ScopedUsageDeps = {
    fetchImpl: fetch,
    homeDir: os.homedir(),
    now: () => Date.now(),
    ...overrides,
