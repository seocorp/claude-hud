...
export function renderUsageLine(
  ...
  const effectiveUsage = Math.max(
    fiveHour ?? 0,
    sevenDay ?? 0,
    ...modelScoped.map((scoped) => scoped.percent ?? 0),
  );
  ...
  if (usageCompact) {
    ...
    const scopedCompactParts = modelScoped.map((scoped) =>
      formatCompactWindowPart(scoped.label, scoped.percent, scoped.resetAt, SEVEN_DAY_WINDOW_MS, timeFormat, colors, usageValueMode));
  ...
  const scopedParts = modelScoped.map((scoped) =>
